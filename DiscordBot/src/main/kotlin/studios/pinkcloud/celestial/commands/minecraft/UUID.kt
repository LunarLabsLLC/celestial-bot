...
class UUID : ApplicationCommand(){
    ...
    @JDASlashCommand(
        ...
        name = "uuid",
        description = "Get the Minecraft UUID of a player"
    )

    fun onSlashCommand(event: SlashCommandInteractionEvent, @AppOption(name = "ign", description = "Player's Minecraft IGN",) ign: String) {
        val ign = event.getOption("ign")?.asString ?: return
        val uuid = fetchPlayerUUID(ign)
        val embed = EmbedBuilder()
            .setTitle("Player UUID")
            .setColor(Color.decode(Colors.FIRST_COLOR.hexCode))
            .setDescription("UUID of player `$ign`")
